...
#Region " CÛdigo generado por el DiseÒador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Label1.Size = New System.Drawing.Size(512, 99)
...
#End Region
    ...
    Private Sub BorraTicketHandler(ByVal Mensaje As String, ByVal TmpAuxVta As Boolean, ByVal TmpAuxVta1 As Boolean, ByVal ECVenta As Boolean, ByVal LigasFolios As Boolean, ByVal ECVentaDete As Boolean, ByVal ECVentaDet As Boolean, ByVal ECDetVenta As Boolean)
        ...
        If ECVenta Then
            Try
                Base.Ejecuta("delete from ecventa where ven_id=" & fo.xven & " and ven_usuario='" & "000" & Globales.caja.Substring(Len(Globales.caja) - 1, 1) & "'", sCadenaConexionSQL)
                BorraECVenta = True
            Catch ex As Exception
                BorraECVenta = False
            End Try
        ...
        If LigasFolios Then
            Try
                Base.Ejecuta("delete from Ligasfolios where ven_id=" & fo.xven, sCadenaConexionSQL)
        ...
        If ECVentaDete Then
            Try
                Base.Ejecuta("delete from ecventadete where dve_venta=" & fo.xven, sCadenaConexionSQL)
        ...
        If ECVentaDet Then
            Try
                Base.Ejecuta("delete from ecdetventa where dve_venta=" & fo.xven, sCadenaConexionSQL)
        ...
        If ECDetVenta Then
            Try
                Base.Ejecuta("delete from ecventadet where dve_venta=" & fo.xven, sCadenaConexionSQL)
        ...
        Else
            ...
            If Pendientes <> "" Then
                Mensaje2 = " No pudo borrarse la informaciÛn de " & Pendientes & ", "
            End If
            If Borradas <> "" Then
                Mensaje2 = Mensaje2 & " sÛlo de " & Borradas & "."
            Else
    ...
    Private Sub BorraFormaPagoHandler(ByVal Ticket As Integer)
        Try
            Base.Ejecuta("delete from ECFormaPago where referencia=" & Ticket, sCadenaConexionSQL)
            Try
                Base.Ejecuta("delete from ECTarjetas where Tar_VentaID=" & Ticket, sCadenaConexionSQL)
                Try
                    Base.Ejecuta("delete from ECTransfer where Tra_VentaID=" & Ticket, sCadenaConexionSQL)
                    Try
                        Base.Ejecuta("delete from ECcuentasXCob where CxC_Folio=" & Ticket, sCadenaConexionSQL)
                        Try
                            Dim SQL As String
                        ...
                        Catch ex As Exception
                            MsgBox("Error CrÌtico - Notifique a Jaime Burciaga." & Chr(13) & Chr(13) & "No pudo actualizarse saldos de cliente en ECDetClienteEmp; sÛlo se borrÛ progreso de tablas ECFormaPago, ECTarjetas, ECTransfer y ECCuentasXCob.", MsgBoxStyle.Critical, "Punto de Venta")
                        End Try
                    Catch ex As Exception
                        MsgBox("Error CrÌtico - Notifique a Jaime Burciaga." & Chr(13) & Chr(13) & "No pudo borrarse la informaciÛn de la tabla ECCuentasXCob, ni actualizar saldos de cliente en ECDetClienteEmp; sÛlo de ECFormaPago, ECTarjetas y ECTransfer.", MsgBoxStyle.Critical, "Punto de Venta")
                    End Try
                Catch ex As Exception
                    MsgBox("Error CrÌtico - Notifique a Jaime Burciaga." & Chr(13) & Chr(13) & "No pudo borrarse la informaciÛn de las tablas ECTransfer, ECCuentasXCob, ni actualizar saldos de cliente en ECDetClienteEmp; sÛlo de ECFormaPago y ECTarjetas.", MsgBoxStyle.Critical, "Punto de Venta")
                End Try
            Catch ex As Exception
                MsgBox("Error CrÌtico - Notifique a Jaime Burciaga." & Chr(13) & Chr(13) & "No pudo borrarse la informaciÛn de las tablas ECTarjetas, ECTransfer, ECCuentasXCob, ni actualizar saldos de cliente en ECDetClienteEmp; sÛlo de ECFormaPago.", MsgBoxStyle.Critical, "Punto de Venta")
            End Try
        Catch ex As Exception
            MsgBox("Error CrÌtico - Notifique a Jaime Burciaga." & Chr(13) & Chr(13) & "No pudo borrarse la informaciÛn de las tablas ECFormaPago, ECTarjetas, ECTransfer, ECCuentasXCob, ni actualizar saldos de cliente en ECDetClienteEmp.", MsgBoxStyle.Critical, "Punto de Venta")
        End Try
    End Sub
    ...
    Private Sub btn_continuar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_continuar.Click
        ...
        ' 2. INICIO DE LA TRANSACCI”N MAESTRA
        Using Conn As New SqlConnection(sCadenaConexionSQL)
            ...
            ' Iniciamos la transacciÛn. A partir de aquÌ, nada es definitivo hasta el Commit.
            Using Trans As SqlTransaction = Conn.BeginTransaction("VentaCompleta")

                Try
                    ' Creamos un comando vinculado a la transacciÛn
                    Using Cmd As SqlCommand = Conn.CreateCommand()
                        Cmd.Transaction = Trans
                        Cmd.CommandTimeout = 120 ' Damos tiempo suficiente

                        ' ---------------------------------------------------------
                        ' FASE A: GUARDAR EL TICKET (Llamamos a la funciÛn privada nueva)
                        ' ---------------------------------------------------------
                    ...
                    ' 3. SI LLEGAMOS AQUÕ SIN ERRORES -> CONFIRMAMOS TODO
                    Trans.Commit()
                    ' °LISTO! AquÌ se guardÛ todo de golpe.

                Catch ex As Exception
                    ' 4. SI ALGO FALL” -> DESHACEMOS TODO (ROLLBACK)
                    Try
                        Trans.Rollback()
                    Catch
                        ' Ignoramos error si la conexiÛn ya estaba cerrada
                    End Try

                    MsgBox("Error crÌtico al procesar la venta. No se guardÛ nada." & vbCrLf &
        ...
        ' =========================================================
        ' 5. PROCESOS POST-VENTA (Fuera de la transacciÛn)
        ' =========================================================
        ...
        Try
            ...
            ' A. PREPARAR IMPRESI”N PRINCIPAL
            ...
            ' Esta clase prepara la colecciÛn necesaria para el segundo ticket y otros procesos
            Dim imprimevirtual As New previrtual(iID)

            ' C. IMPRIMIR TICKET NORMAL
            fo.imprimeTicket(preImpresion, iID)

            ' D. IMPRIMIR SEGUNDO TICKET (Vales/MercancÌa Especial)            
            If HayVales Then
                ' AQUÕ USAMOS LA COLECCI”N DE imprimevirtual COMO EN TU ORIGINAL
                Dim vImpresion As New Impresion(imprimevirtual.COLECCION, iID)
                vImpresion.imprime(True)
            End If

            ' E. IMPRIMIR PAGAR… (CrÈdito)
            If fo.pagare Then
            ...
            If fo.sipuntos Then
                ...
                oImpresion.imprime(False) ' False = No abrir cajÛn (ya se abriÛ con el ticket normal)
            End If

            ' G. TIEMPO AIRE (TAE)
            If fo.estiempoaire Then
                If Not ProcesarTiempoAire(iID) Then
                    ' Si el TAE falla pero la venta ya se cobrÛ, aquÌ avisas
                End If
            End If
        ...
        Catch ex As Exception
            MsgBox("Venta guardada exitosamente, pero hubo un error en la impresiÛn: " & ex.Message, vbExclamation)
        End Try
    ...
    End Sub
    ' Esta funciÛn va dentro de tu formulario o clase de lÛgica
    Private Function GuardaTicket_Transaccional(ByVal Cmd As SqlCommand,
        ...
        ' B) ECDETVENTA (ImpresiÛn - Agrupado por Clave)
        ...
        ' C) ECVENTADETE (Vales - Sin Agrupar / CronolÛgico)
        ...
        With fo.FpArticulos.ActiveSheet
            ...
            For i As Integer = 0 To .RowCount - 1
                ...
                Dim FactorAplicado As Decimal = CDec(Val(.Cells(i, ColVenta.ColFactor).Value))
                If FactorAplicado <= 0D Then FactorAplicado = 1D

                Dim CantidadCapturada As Decimal = Cantidad
                Dim CantidadReal As Decimal = CantidadCapturada * FactorAplicado
                ...
                Dim ClaveInv As String = UPC & "|" & F6.ToString() & "|" & FactorAplicado.ToString("0.####")

                ' ------------------------------------------------------------------
                ' A) INVENTARIO (ECVENTADET) - AGRUPAR POR UPC PURO
                ' ------------------------------------------------------------------
                ' AquÌ sumamos TODO (Normales + Vales). 
                ' Resultado: 1 solo renglÛn por UPC con la cantidad total a descontar.
                If Not DicInventario.ContainsKey(ClaveInv) Then
                    DicInventario.Add(ClaveInv, New ItemAcumulado With {
                        .UPC = UPC, .Clave = ArtClave, .Descripcion = NomLargo,
                         .CostoU = CostoU, .Familia = Familia,
                ...
                With DicInventario(ClaveInv)
                    .Cantidad += CantidadReal
                    .CantidadCapturada += CantidadCapturada
                    .Total += TotalLinea
                    .ImporteIVA += ImpIVA
                    .ImporteIEPS += ImpIEPS
                    .PrecioU = If(.Cantidad > 0D, .Total / .Cantidad, 0D)
                End With

                ' ------------------------------------------------------------------
                ' B) IMPRESION (ECDETVENTA) - AGRUPAR POR CLAVE
                ' ------------------------------------------------------------------
        ...
        For Each kvp In DicInventario
            ...
            dtUPC.Rows.Add(itm.Clave, itm.UPC, itm.PrecioU, itm.Cantidad, itm.Total,
                   ...
                   itm.CantidadCapturada, itm.FactorAplicado)
            RenglonInv += 1
        Next


        ' 4.2 ImpresiÛn (ECDETVENTA)
        ...
        ' B) TVP 1: ECVENTADET (1 RenglÛn por UPC - M·xima velocidad Inventario)
    ...
    Public Sub SacaProporcionP1P2_Transaccional(ByVal Cmd As SqlCommand, ByVal NumTicket As Integer, ByVal MontoTotal As Decimal)
        ...
        ' 1. LIMPIEZA DE PAR¡METROS (Vital porque el Cmd viene "sucio" de los inserts anteriores)
        Cmd.Parameters.Clear()
        Cmd.Parameters.AddWithValue("@folio", NumTicket)

        ' 2. CALCULO DIRECTO SOBRE LA VENTA REAL (ECVENTADET)
        ' Nota Importante: Cambiamos "THEN 1" por "THEN dve_cantidad" porque ahora los renglones estan agrupados.
        ...
        Using Rdr As SqlDataReader = Cmd.ExecuteReader()
            ...
            End If
        End Using
        ' El Reader se cierra aquÌ y libera el Cmd
        ...
        ' 4. INSERTADO EN TABLA DE PROPORCI”N
        ...
        Cmd.Parameters.AddWithValue("@usu", Globales.nombreusuario) ' Ojo: Aseg˙rate que esta variable tenga valor, o usa el par·metro Usuario que recibÌa la funciÛn principal
    ...
    Private Sub GuardaPagos_Transaccional(ByVal Cmd As SqlCommand,
        ...
        ' Formateamos la fecha a String seguro para SQL (A√±oMesDia Hora:Min:Seg.Miliseg)
        ' Esto evita problemas de configuraci√≥n regional en el servidor.
        Dim FechaStr As String = Fecha.ToString("yyyyMMdd HH:mm:ss.fff")

        ' Aseguramos que el comando est√© limpio de par√°metros anteriores
        Cmd.Parameters.Clear()
        ...
        With fo.FpFormasPago.ActiveSheet
            ...
            For iRen = 0 To .RowCount - 1
                ...
                ' Validaciones b√°sicas para no tronar por celdas vac√≠as
                ...
                ' =========================================================
                ' A. INSERCI√ìN GENERAL (Tabla ecformapago)
                ' =========================================================
                ...
                ' =========================================================
                ' B. L√ìGICA ESPEC√çFICA POR TIPO DE PAGO
                ' =========================================================

                ' CASO 1: TARJETA (Cr√©dito o D√©bito)
                If TipoPago = "Tarjeta" Then
                    ' Nota: Replicamos tu l√≥gica de usuario: '00' + Usuario + '0'
                    ...
                    ' CASO 2: COMISI√ìN TARJETA (Actualiza el registro anterior)
                ...
                ElseIf TipoPago = "Transfer" Then
                    ...
                    ' CASO 4: CR√âDITO (Pagar√© / Cuentas por Cobrar)
        ...
        ' =========================================================
        ' C. ACTUALIZACI√ìN DE SALDO CLIENTE (Solo si hubo cr√©dito)
        ' =========================================================
        If HuboCredito Then
    ...
    Public Sub GuardaTicketCliente_Transaccional(ByVal Cmd As SqlCommand, ByVal ClienteID As Integer, ByVal TicketID As Integer)
        ' Limpiamos par·metros previos del comando que venimos reutilizando
        Cmd.Parameters.Clear()
        ...
        ' AsignaciÛn de valores
        Cmd.Parameters.AddWithValue("@cliente", ClienteID)
        Cmd.Parameters.AddWithValue("@ticket", TicketID)

        ' Ejecutamos. Al usar 'Cmd', esto ocurre dentro de la transacciÛn maestra.
        Cmd.ExecuteNonQuery()
    End Sub
    ...
    Private Sub ImprimirPagareCredito(ByVal iID As Integer)
        ...
        Using xConLocal As New SqlConnection(Globales.sCadenaConexionSQL)
            Try
                ...
                End Using
            Catch ex As Exception
                MsgBox("Error leyendo datos del cliente para pagarÈ: " & ex.Message)
            End Try
        End Using
    ...
    Private Function ProcesarTiempoAire(ByVal VentaID As Integer) As Boolean
        ...
        If Exito Then
            ...
            Return True
        Else
            MsgBox("El tiempo aire NO se acreditÛ. Devuelva el dinero.", vbExclamation)
            Return False
        End If
    End Function
    Private Sub btn_continuar_ClickOriginal(ByVal sender As Object, ByVal e As System.EventArgs) 'Handles btn_continuar.Click
        Dim sSql As String = ""
        ...
        With fo.FpArticulos.ActiveSheet
            While valido = 0 And Counter < 3
                Base.daQuery("exec dasigfolio '" & lcaja & "'", sCadenaConexionSQL, dsc, "Fo")
                ...
                'Base.Ejecuta("exec dasigfolio '" & lcaja & "'", sCadenaConexionSQL)
                'sSql = "select foliovta from tmpconsecu where caja='" & lcaja & "'"
                'Base.daQuery(sSql, sCadenaConexionSQL, dsc, "tablahel")
                ...
                'Base.daQuery(SQL, sCadenaConexionSQL, dsc, "FolioSerie")
        ...
        'Dim oPreImpre As New preImpresion(Me.tx_cambio.Text, fo.FpArticulos, Globales.sDireccion, Globales.sNominaEmpleado, Globales.NombreEmpleado, fo.LbTotal.Text.Replace(",", ""), iID, sCadenaConexionSQL)
        'preImpresion = oPreImpre.COLECCION


        'Dim imprimevirtual As New previrtual(fo.FpArticulos, iID)
        fo.imprimeTicket(preImpresion, iID)
        ...
        If fo.estiempoaire Then
            ...
            '-- 1. Solicitar TAE
            '-- 2. Cerrar la transacciÛn de Venta con estatus OPERADO
            '-- 3. Validar los estatus en un LOOP de 5 intentos cada 3 segundos
            '-- 4. Si al terminar el LOOP la respuesta NO es satifactoria CANCELAR venta y enviar por Switch Venta y CancelaciÛn
            '-- 5. Si la venta es satisfactoria actualizar el # de autorizaciÛn de la Venta y enviar por Switch
            ...
            Do While Sw_Consultar
                ...
                Select Case Estatus_TAE_CONF
                    ...
                    Case "17"   '-- TransacciÛn NO encontrada
                        Sw_Consultar = False
            ...
            If Estatus_TAE_CONF = "00" Then
                Base.Ejecuta("UPDATE ecdetventatel  Set dve_folio=" & FOLIOAUT.ToString & " WHERE dve_venta=" & iID.ToString & "", sCadenaConexionSQL)
                Dim opretiempo As New pretiempoaire(iID)
                fo.imprimetaire(opretiempo.COLECCION)
        ...
        fo.segundoticket = False
        For l = 0 To fo.FpArticulos.ActiveSheet.RowCount - 1
            If fo.FpArticulos.ActiveSheet.Cells(l, 5).Text = "*" Then
        ...
        If fo.pagare = True Then
            ...
            ' ---------------------------------------------------------
            ' PASO 1: CONEXI”N LOCAL SEGURA (Arquitectura Nueva)
            ' ---------------------------------------------------------
            Using xConLocal As New SqlConnection(Globales.sCadenaConexionSQL)
                Try
                    xConLocal.Open()
                    ...
                    Using comando As New SqlCommand()
                        comando.Connection = xConLocal
                        ' Nota: Asumo que 'fo' es el formulario. Si este cÛdigo est· DENTRO del formulario, cambia 'fo.' por 'Me.'
                        comando.CommandText = "Select cli_nombre, cli_direccion, '' as cli_telefono, dcli_saldo as cli_saldo, cli_clave as id_credito " &
                                  "from ecclientes inner join ecdetclienteemp on dcli_clave=cli_clave " &
                                  "where dcli_empresa=1 and cli_clave=" & fo.clientex

                        Using rdr As SqlDataReader = comando.ExecuteReader()
                            If rdr.Read() Then
                                ' Usamos .ToString() para proteger contra valores Nulos en BD
                                deudor = rdr(0).ToString()
                                direccion = rdr(1).ToString()
                                telefono = rdr(2).ToString()
                                saldo = rdr(3).ToString()
                                credito = rdr(4).ToString()
                            End If
                        End Using ' Se cierra el Reader autom·ticamente
                    End Using ' Se cierra el Comando autom·ticamente

                Catch ex As Exception
                    MsgBox("Error al obtener datos del cliente: " & ex.Message)
                End Try
            End Using ' Se cierra la ConexiÛn autom·ticamente (Connection Pooling)

            ' ---------------------------------------------------------
            ' PASO 2: L”GICA DE INTERFAZ (GRID)
            ' Esto no cambia porque solo lee de la pantalla, no de la BD
            ' ---------------------------------------------------------
            Dim totaluco As Double = 0
            Dim i As Integer
            ...
            With fo.FpFormasPago.ActiveSheet
                For i = 0 To .RowCount - 1
                    If .Cells(i, 0).Value IsNot Nothing AndAlso .Cells(i, 0).Value.ToString() = "Credito" Then
                        ' AgreguÈ validaciÛn IsNumeric para evitar errores si la celda est· vacÌa
                        If IsNumeric(.Cells(i, 1).Value) Then
                            totaluco += CDbl(.Cells(i, 1).Value)
                        End If
                    End If
                Next
            End With

            ' Limpieza visual (estaba en tu cÛdigo original)
            ' OJO: Esta lÌnea 'fo.tb_total.Text.Replace...' no hace nada si no se asigna. 
            ' Probablemente querÌas: fo.tb_total.Text = fo.tb_total.Text.Replace(",", "")
            ' La dejo como estaba para no alterar tu lÛgica, pero revÌsala.
            fo.tb_total.Text.Replace(",", "")

            ' ---------------------------------------------------------
            ' PASO 3: IMPRESI”N
            ' ---------------------------------------------------------
            ' Asumo que iID viene de alg˙n lado (probablemente global o par·metro)
        ...
        Else
            If Sw_Cancelar = True Then
                Base.Ejecuta("UPDATE ECVENTA SET VEN_STATUS=2,VEN_DSCUSUARIO='" & Globales.nombreusuario & "',VEN_FECHACANC=GETDATE() , VEN_MOTIVO='" & " TIEMPO AIRE QUE NO SE ACREDITO " & "' WHERE VEN_ID=" & iID, sCadenaConexionSQL)
                '                Base.Ejecuta("EXEC APLICAVENTAINVENTARIO " & TICKET & ",4", sCadenaConexionSQL)DIF
                '                    Base.Ejecuta("EXEC APLICAVENTAINVENTARIO " & TICKET & ",4", sCadenaConexionSQL)
                MsgBox("No se aplicÛ el TIEMPO AIRE AL CLIENTE, DEVUELVA EL DINERO  , LA OPERACION SE HA CANCELADO ", MsgBoxStyle.Information)
            Else
    ...
    Private Class ItemAcumulado
        ...
        ' Valores Unitarios/Fijos (Se toman del ˙ltimo registro o se promedian, usualmente son fijos por UPC)
        ...
        Public FactorAplicado As Decimal = 1
