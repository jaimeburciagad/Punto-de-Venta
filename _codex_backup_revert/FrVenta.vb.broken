...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.PictureBox8.BackgroundImage = Global.ECVENTA4.My.Resources.Resources.money_transfer_icon_40389
        ...
        Me.PictureBox21.BackgroundImage = Global.ECVENTA4.My.Resources.Resources._1485477061_cart_785851
        ...
        Me.PictureBox22.BackgroundImage = Global.ECVENTA4.My.Resources.Resources.ValesIcon
        ...
        Me.PictureBox24.BackgroundImage = Global.ECVENTA4.My.Resources.Resources.Loan
        ...
        Me.PictureBox23.BackgroundImage = Global.ECVENTA4.My.Resources.Resources.CheckIcon
        ...
        Me.PictureBox25.BackgroundImage = Global.ECVENTA4.My.Resources.Resources._1485477165_credit_card_78583
        ...
        Me.PictureBox13.BackgroundImage = Global.ECVENTA4.My.Resources.Resources.CashIcon
...
#End Region
        ...
        Using xConLocal As New SqlConnection(Globales.sCadenaConexionSQL)
            Try
                ...
                Using transaction As SqlTransaction = xConLocal.BeginTransaction("GuardarPagos")
                    ...
                    ' Creamos el comando que ejecutará todas las órdenes dentro de la transacción
    ...
    Public Sub agregaArticulo(ByVal Item As ArticuloVenta)
        ...
        Dim ClienteID As Integer = Item.ClienteID
        If ClienteID = 0 Then ClienteID = CInt(Val(TxtControl.Text))

        If Item.EsKilos OrElse Item.CargandoPendiente OrElse Item.EsCancelacion Then
            sCant = Item.Cantidad.ToString()
        End If
        ...
        With FpArticulos.ActiveSheet
            ...
            If .RowCount > 1 Then
                If .Cells(iRen - 1, ColVenta.ColUPCInv).Text.Trim = Item.UPC AndAlso Not Item.EsCancelacion AndAlso CDec(Val(.Cells(iRen - 1, ColVenta.ColCantidad).Value)) > 0D AndAlso Not Item.EsKilos AndAlso Not Item.CargandoPendiente Then
                    iRen -= 1
                    cantacuma = CDec(Val(.Cells(iRen, ColVenta.ColCantidad).Value))
                    sCant = (cantacuma + 1D).ToString()

                    If Not Item.CargandoPendiente Then
                        SQL = "update tmpauxvta2 set cantidad=" & sCant & ", FueConF1=" & valF1 & ", FueConF2=" & valF2 & ", FueConF3=" & valF3 & ", FueConF6=" & valF6 & " where renglon=" & iRen & " and usuario='" & Globales.caja & "'"
                        Base.Ejecuta(SQL, sCadenaConexionSQL)
                    End If

                    .Cells(iRen, ColVenta.ColCantidad).Value = CDec(sCant)
                    .Cells(iRen, ColVenta.ColPrecio).Value = If(Item.TipoOferta = 0, Item.Precio1, Item.PrecioOferta1)
                    .Cells(iRen, ColVenta.ColTotal).Value = CDec(.Cells(iRen, ColVenta.ColPrecio).Value) * CDec(sCant)
                    .Cells(iRen, ColVenta.ColDescripcion).Value = Item.Descripcion
                    .Cells(iRen, ColVenta.ColUPCInv).Value = Item.UPC
                    .Cells(iRen, ColVenta.ColFIVA).Value = Item.IVA
                    .Cells(iRen, ColVenta.ColFIEPS).Value = Item.IEPS
                    .Cells(iRen, ColVenta.ColArtClave).Value = Item.Clave
                    .Cells(iRen, ColVenta.ColNomLargo).Value = Item.NombreLargo
                    .Cells(iRen, ColVenta.ColFamilia).Value = Item.Familia
                    .Cells(iRen, ColVenta.ColCostoCap).Value = Item.CostoCap
                    .Cells(iRen, ColVenta.ColTipoOferta).Value = Item.TipoOferta
                    .Cells(iRen, ColVenta.ColPrecio1).Value = Item.Precio1
                    .Cells(iRen, ColVenta.ColF1).Value = valF1
                    .Cells(iRen, ColVenta.ColF2).Value = valF2
                    .Cells(iRen, ColVenta.ColF3).Value = valF3
                    .Cells(iRen, ColVenta.ColF6).Value = valF6
                    .Cells(iRen, ColVenta.ColFactor).Value = Item.Factor
                    .Cells(iRen, ColVenta.ColVale).Value = If(Item.TraeVale, "*", "")
                    .RemoveRows(iRen + 1, 1)
                    .ActiveRowIndex = iRen
                Else
                    If Not Item.CargandoPendiente Then
                        SQL = "insert into tmpauxvta2 (renglon,usuario,articulo,cantidad,tipo,FueConF1,FueConF2,FueConF3,ClienteID,ArtClave,TraeVale,Factor,FueConF6) values(" & _
                              iRen & ",'" & Globales.caja & "','" & Item.UPC & "'," & If(Item.EsKilos, 0, CDec(sCant)) & "," & Item.TipoVenta & "," & valF1 & "," & valF2 & "," & valF3 & "," & ClienteID & ",'" & Item.Clave & "','" & If(Item.TraeVale, "*", "") & "'," & Item.Factor & "," & valF6 & ")"
                        Base.Ejecuta(SQL, sCadenaConexionSQL)
                    End If
                    ...
                    If Item.EsKilos Then
                        .Cells(iRen, ColVenta.ColCantidad).Value = 0D
                        .Cells(iRen, ColVenta.ColPrecio).Value = 0D
                        .Cells(iRen, ColVenta.ColTotal).Value = 0D
                    Else
                        .Cells(iRen, ColVenta.ColCantidad).Value = CDec(sCant)
                        .Cells(iRen, ColVenta.ColPrecio).Value = If(Item.TipoOferta = 0, Item.Precio1, Item.PrecioOferta1)
                        .Cells(iRen, ColVenta.ColTotal).Value = CDec(.Cells(iRen, ColVenta.ColPrecio).Value) * CDec(sCant)
            ...
            If virtual = 1 And Not estiempoaire Then
                .Cells(.ActiveRowIndex, ColVenta.ColVale).Value = "*"
                virtual = 0
            End If
        End With
        ...
        CONTROLPRECIOS(UPCUPC:=Item.UPC,
                       ...
                       SCANT:=If(Item.EsKilos, If(Item.CargandoPendiente, CDec(Val(sCant)), 0D), CDec(Val(sCant))),
                       SCANTANT:=cantacuma,
                       RENGLON:=FpArticulos.ActiveSheet.ActiveRowIndex,
                       ArtClave:=Item.Clave,
                       ...
                       CantidadCobradas:=Item.CantidadCobradas + CInt(Math.Abs(Item.Cantidad)),
    ...
    Private Sub CONTROLPRECIOS(ByVal UPCUPC As String, ByVal DPRECIO1 As Decimal, ByVal DPRECIO2 As Decimal, ByVal DPRECIO3 As Decimal, ByVal TOPE As Decimal, ByVal SCANT As Decimal, ByVal SCANTANT As Decimal, ByVal RENGLON As Integer, ByVal ArtClave As String, ByVal Optional CargandoPendiente As Boolean = False, Optional ByVal CantidadDisponible As Decimal = -1, Optional ByVal PrecioOferta1 As Decimal = -1, Optional ByVal PrecioOferta2 As Decimal = -1, Optional ByVal PrecioOferta3 As Decimal = -1, Optional ByVal CantidadCobradas As Integer = 0, Optional ByVal TipoOferta As Integer = 0)
        ...
        If Not CargandoPendiente Then
            Try
                ' A) Verificar si existe (Lectura rápida)
    ...
    Private Sub tx_cantidad_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles tx_cantidad.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            ' Validación básica de número (evita crashes)
    ...
    Private Sub ProcAgregaArticulo(ByVal Optional CargandoPendiente As Boolean = False, ByVal Optional CantPend As Double = 1, ByVal Optional PendFueConF1 As Integer = 0, ByVal Optional PendFueConF2 As Integer = 0, ByVal Optional PendFueConF3 As Integer = 0, Optional PendTraeVale As String = "", Optional PendFueConF6 As Integer = 0)
        ...
        ' =================================================================================
        ' NOTA: Eliminamos "and art_activo=1" del WHERE para traerlo aunque esté bloqueado
                    ...
                    If NuevoTipoVenta = 3 Then
                        ...
                        Else
                            ...
                            If ResultadoBusqueda > 0 Then
                                ...
                                ' Actualizamos visualmente el nombre (necesitarás traer el nombre del buscador o BD)
